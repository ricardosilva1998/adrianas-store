...
**Announcement bar (`globals.banner`)**: As of 2026-05-15 the bar uses the shape `{ enabled, contentHtml, bgHex, textHex, dismissible, contentVersion }`. The legacy shape (`{ text, linkUrl, bgColor: "rosa"|"ink" }`) is converted on read inside `src/lib/config-server.ts:readFromDb()` via `legacyBannerMigration` from `src/lib/legacy-banner.ts` — kept out of `config.ts` so the schema module stays free of `node:crypto` and ships safely to client bundles. Storefront rendering lives in `src/components/AnnouncementBar.astro` (mounted by `BaseLayout` before `<Header>`); the pure helper `getAnnouncementRender` in `src/lib/announcement-bar.ts` decides `shouldRender` (via `isEmptyHtml`) and returns the sanitized HTML. Sanitization uses `sanitizeAnnouncement` (`src/lib/sanitize.ts`) — a tight allowlist (p/br/strong/em/u/a/span only) with a scoped `uponSanitizeAttribute` hook that accepts only `style="color: …"` and rejects everything else; the flag is per-call so `sanitizeHtml` (product descriptions, TextBlock) is unaffected. The admin edits the bar at `/admin/globals → Banner` with `<RichTextEditor mode="inline">` and two `<ColorPicker presets={…}>` controls; `contentVersion` is a 12-char base64url SHA-256 hash of `contentHtml`, computed async on the client via `hashContentAsync` in `src/lib/banner-hash.ts` (isomorphic, no `node:crypto`) and re-hashed server-side on every `PUT /api/admin/site-config` via `hashContentSync`. The dismiss button keys on `contentVersion` in `localStorage`, so editing the bar's content automatically un-dismisses it for all visitors. Migration script: `scripts/migrate-banner-shape.ts` (idempotent; run once post-deploy via `railway run`).
...
- **TipTap inline mode**: `RichTextEditor` accepts an optional `mode="inline"` (default `"full"`) that strips heading/list/blockquote/alignment/strike from both the toolbar and the editor extensions. Use this on single-line surfaces like the announcement bar where block-level formatting doesn't apply; pair with `sanitizeAnnouncement` on the output.
...
### 2026-05-15 15:33 — claude (inline /executing-plans)
**Task:** Deploy announcement bar rewrite — rich text TipTap inline, hex livre para fundo e texto, dismiss versionado por contentVersion + migração do shape em produção
**Files:** src/lib/legacy-banner.ts, src/lib/legacy-banner.test.ts, src/lib/banner-hash.ts, src/lib/announcement-bar.ts, src/lib/announcement-bar.test.ts, src/lib/config.ts, src/lib/config-server.ts, src/lib/sanitize.ts, src/lib/sanitize.test.ts, src/components/AnnouncementBar.astro, src/components/Header.astro, src/components/admin/RichTextEditor.tsx, src/components/admin/RichTextEditor.test.tsx, src/components/admin/ColorPicker.tsx, src/components/admin/GlobalsEditor.tsx, src/components/islands/AnnouncementDismissButton.tsx, src/components/islands/AnnouncementDismissButton.test.tsx, src/layouts/BaseLayout.astro, src/pages/api/admin/site-config.ts, scripts/migrate-banner-shape.ts, docs/superpowers/specs/2026-05-15-announcement-bar-design.md, docs/superpowers/plans/2026-05-15-announcement-bar.md, CLAUDE.md
**Decisions:**
- 14 commits (f64472e → df50c2c + 70e740a plano) pushed origin/main @ 70e740a (e91b19f..70e740a); 195/196 testes verde (1 skipped pré-existente), build 1.65s — todos os steps do plano executados via skill superpowers:executing-plans
- Shape novo: `{ enabled, contentHtml, bgHex, textHex, dismissible, contentVersion }`. Legacy → novo via `legacyBannerMigration` chamado em `config-server.ts:readFromDb()` antes do `safeParse` (não como Zod preprocess) — mantém `config.ts` puro sem `node:crypto`, evita partir bundles client. `EMPTY_CONTENT_VERSION_LITERAL = "47DEQpj8HBSa"` hardcoded em `config.ts` matches `hashContentSync("")` (snapshot verificado em runtime na migração de produção)
- Hash dividido em dois ficheiros: `legacy-banner.ts` (server-only, `createHash` de `node:crypto` em `hashContentSync`) e `banner-hash.ts` (isomórfico, `globalThis.crypto.subtle` em `hashContentAsync`). Admin React island importa de `banner-hash.ts`. Snapshot determinista `hashContentSync("oi") === "h_YzY0zEsC9i"` cravado em teste
- Sanitização: `sanitizeAnnouncement` com allowlist apertada (p/br/strong/em/u/a/span + href/target/rel/style) e hook `uponSanitizeAttribute` escopado por flag `sanitizingAnnouncement` — só filtra `style=` quando chamado via essa função; `sanitizeHtml` (descrições de produto, TextBlock) fica intacto. Auditoria DB pré-implementação confirmou que os 30+ `style=` em produção são todos `color: rgb(...)` — scope mantido por defesa contra futuro
- RichTextEditor ganha prop `mode="full"|"inline"`; em modo inline o StarterKit perde heading/listas/blockquote/codeBlock/horizontalRule/strike e TextAlign omitido; toolbar reduz para B/I/U/cor/link/limpar. Modo full default — zero impacto em ProductForm/BlockForm/Textarea de GlobalsEditor
- ColorPicker ganha prop opcional `presets={[{label,hex}]}` — fila de swatches 24×24 ring-ink quando activo. BannerForm passa presets derivados de `config.theme.colors` (Primária, Neutra, Destaque?, Branco, Preto)
- AnnouncementBar montado em BaseLayout antes do Header; helper puro `getAnnouncementRender` testado; dismiss button `client:load` keya em `drisclub-banner-dismissed-<contentVersion>` no localStorage e esconde o ancestor `[data-announcement-version=...]` via `display:none`
- PUT /api/admin/site-config faz re-hash defensivo do `contentVersion` server-side antes do `db.update` — cliente nunca é fonte de verdade
- payment.md left untracked como por convenção do projecto; secrets scan clean (único match foi "token" em CLAUDE.md ref a preview tokens — doc, não secret)
- Railway picked up GitHub push automatically; deploy correu OK — server listening on port 3000 at 12:48:01; 60s log window clean (migration NOTICEs esperadas — "already exists, skipping"); sem nova migração SQL; sem env-var changes
- Migration script `scripts/migrate-banner-shape.ts` corrido em produção via `railway run -s adrianas-store -- bash -c 'DATABASE_URL=$DATABASE_PUBLIC_URL npx tsx ...'` — output: `OK — banner migrado` (shape antigo `text=""` legacy convertido para novo com `contentVersion: "47DEQpj8HBSa"` matching o literal). 2ª execução: `Shape novo já em uso. Skip.` — idempotência confirmada
**Open:** none

### 2026-05-15 18:59 — claude (inline /executing-plans)
**Task:** Estatísticas de visitas no /admin (tabela page_views + tracking anónimo + 4 widgets dashboard) + fix JWT_SECRET + apple-touch-icon para favicon mobile
**Files:** src/db/schema.ts, src/db/migrations/0018_page_views.sql, src/db/migrations/meta/_journal.json, src/lib/tracking.ts, src/lib/tracking.test.ts, src/lib/analytics.ts, src/middleware.ts, src/components/admin/VisitsChart.tsx, src/components/admin/VisitsChart.test.tsx, src/components/admin/TopVisitedBlock.astro, src/pages/admin/index.astro, src/layouts/BaseLayout.astro, docs/superpowers/specs/2026-05-15-admin-visit-stats-design.md, docs/superpowers/plans/2026-05-15-admin-visit-stats.md, CLAUDE.md
**Decisions:**
- 11 commits (4958bc6 spec → 36985fa apple-touch-icon) pushed origin/main; 227/228 testes verde (+24 novos: 21 tracking puros + 3 VisitsChart); npm run build success em ~1.7s a cada iteração — todos os steps do plano executados via skill superpowers:executing-plans
- Schema: `page_views(id serial PK, path text, visitor_hash text 16 chars, viewed_at timestamptz default now)` + 2 índices (`viewed_at`, `(path, viewed_at)`). Migração 0018 aplicada automaticamente no startup do container Railway (logs: `[migrate] ✔ Migrations concluídas` a 17:54:59 UTC)
- Tracking middleware fire-and-forget em `src/middleware.ts` antes do `return next()` final; só dispara para path patterns explícitos (`/`, `/catalogo/[slug]`, 6 institucionais); excluí admin/api/catalogo-listagem/categoria/carrinho/checkout/conta/preview/draft/non-GET/non-html/bots (regex `/bot|crawler|spider|preview|monitor|lighthouse|headlesschrome/i`)
- Hash anónimo via `daily rotating salt`: `sha256(JWT_SECRET + YYYY-MM-DD)` como salt + `sha256(salt + ip + ua).slice(0,16)`. **IP nunca persistido em disco**, só passa pela função hash em memória. GDPR-friendly sem cookies nem banner de consentimento. Visitante "regressa novo" no dia seguinte (propriedade desejada)
- **Fix crítico durante smoke test**: spec/plan diziam `AUTH_SECRET` mas o projeto inteiro usa `JWT_SECRET` (verificado via grep em `src/lib/auth.ts` e `src/lib/customer-auth.ts`). Sem o fix, `recordPageView` retornava silenciosamente porque `process.env.AUTH_SECRET` era `undefined`. Commit `af9a645` corrigiu, smoke test passou logo
- Pure helpers (`shouldTrack`, `getClientIp`, `computeVisitorHash`) totalmente isolados de DB via lazy `await import("../db/client")` dentro de `recordPageView` — testes Vitest correm sem `DATABASE_URL` (necessário porque o import top-level de `db/client` lança se a env var não existir)
- Retenção 90d via **cleanup self-healing in-memory**: `let lastCleanupAt = 0` module-level em `analytics.ts`; `maybeCleanupOldPageViews()` chamado fire-and-forget no topo de `getVisitStats()`; corre `DELETE FROM page_views WHERE viewed_at < now() - interval '90 days'` se > 24h desde último. Sem schema novo, sem cron Railway. Idempotente
- 5 queries paralelas em `analytics.ts` via `db.execute(sql.raw(...))` + helper `rowsOf<T>` para normalizar shapes de driver (`{ rows: T[] }` vs `T[]`). Top produtos via `JOIN products ON path = '/catalogo/' || slug`; top páginas via `LEFT JOIN pages ON pages.slug = ltrim(path, '/')` (homepage `/` ↔ slug `""`)
- UI: nova `<section class="mt-12">` em `src/pages/admin/index.astro` com header "Tráfego do site" + 4 KPI cards (Hoje / 7d / 30d / Taxa de conversão 30d) + chart row idêntica à existente — `VisitsChart` (React island, SVG hand-rolled mesmo padrão do `DailyChart`, 2 linhas: rosa #ED7396 pageviews + ink #1f2937 uniques tracejada) e `TopVisitedBlock` (server-rendered Astro, top 5 produtos + top 5 páginas com `humanizePath` fallback)
- Taxa de conversão: encomendas pagas (status ∈ paid/preparing/shipped/delivered, created_at >= now() - 30d) ÷ visitantes únicos 30d. É aproximação (numerador e denominador têm identidades diferentes — não há FK entre orders e page_views). Tooltip no card explicita-o
- Favicon mobile (`36985fa`): `BaseLayout.astro` só emitia `<link rel="icon">`; iOS Safari faz fallback para screenshot quando falta `apple-touch-icon`. Acrescentei `<link rel="apple-touch-icon" href={favicon.url ?? "/favicon.svg"} />` ao lado do existente. **Cache iOS é agressivo — pode demorar restart de Safari ou clear-cache para ver mudar num device que já visitou o site**
- payment.md left untracked como por convenção do projecto; secrets scan clean (matches em prose/test fixtures sobre "secret" e "?preview=token" são referências textuais, não valores reais)
- Railway picked up GitHub push automatically para todos os 3 deploys da sessão (push inicial 9 commits @ 5bb3f6c, fix JWT_SECRET @ af9a645, favicon @ 36985fa); migration 0018 aplicada sem erros
- **Smoke test produção (validado a 17:57:25 UTC)**: `curl -A "Mozilla/5.0 …" https://drisclub.com/` → 1 linha em `page_views` (HTTP 200, registo confirmado via `psql $DATABASE_PUBLIC_URL`). `curl -A "Googlebot/2.1" …` → contagem permaneceu em 1 (bot filtrado). Tabela existe com schema correcto
**Open:** Cache iOS da apple-touch-icon — utilizador deve testar num device limpo ou após clear-cache para confirmar visual. À medida que o tráfego acumula em `page_views`, o `/admin` ganha dados reais nas KPI cards "Hoje/7d/30d" e na taxa de conversão; widget "Top produtos vistos" requer que pelo menos uma página de produto seja visitada
